...
{
    [TestClass()]
    public class DocumentPropertyLastModifiedByTests
    {
        [TestMethod()]
        ...
        public void WriteTest()
        {
            var fixture = new XmlFixture();
            var doc = new WordDoc(fixture.WordSampleCopy);
            var result = doc.Process(new DocumentPropertyLastModifiedBy("Lean RAQA Systems"));
            Assert.IsTrue(result.IsSuccess);
            var resultInspect = doc.Inspect(new DocumentPropertyLastModifiedBy());
            Assert.IsTrue(result.IsSuccess);
            Assert.AreEqual("Lean RAQA Systems", result.Value.State);
        }
        ...
        public void WriteDirTest()
        {
            ...
            var resultDocCollection = manager.Process(new DocumentPropertyLastModifiedBy("Lean RAQA Systems"));
